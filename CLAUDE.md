...
- `lang/ru.js` — МАСТЕР. Всегда обновляем только его при добавлении нового контента.
- `lang/en.js` — ВСЕГДА полная копия ru.js + перевод. Генерируется из ru.js целиком.
- Количество строк en.js ≈ ru.js (допуск ±5 строк). Если расходится — значит en.js устарел.

### Workflow нового контента
...
2. Переводим ВСЕ значения с русского → английский (ключи, HTML теги, числа, эмодзи — без изменений)
3. Меняем `const LANG_RU = {` → `const LANG_EN = {`
4. **Полностью перезаписываем** `lang/en.js` — никаких частичных обновлений
5. Проверяем синтаксис: `node -e "require('./lang/en.js'); console.log('OK')"`

### Запрещено
- Хардкодить текст в index.html (даже временно)
- Обновлять en.js частично (только целиком из ru.js)
- Добавлять новые ключи напрямую в en.js минуя ru.js
